...
const char * const BITCOIN_CONF_FILENAME = "akasyacoin.conf";
...
#else
    const char* pszModule = "akasyacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Akasyacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Akasyacoin";
#else
    // Unix
    return pathRet / ".akasyacoin";
#endif
#endif
}
